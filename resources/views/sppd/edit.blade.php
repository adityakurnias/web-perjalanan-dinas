...
<div class="min-h-screen bg-violet-50 flex items-center justify-center p-4 sm:p-6 lg:p-8">
    <div class="max-w-2xl w-full bg-white rounded-2xl shadow-lg p-8 space-y-6">
        <h2 class="text-3xl font-bold text-center text-[#6750A4]">Edit SPPD</h2>

        <form method="POST" action="{{ route('sppd.update', $sppd->id) }}" class="space-y-6">
            @csrf
            @method('PUT')

            <div>
                <label for="tujuan" class="block text-sm font-medium text-[#6750A4]">Tujuan Perjalanan</label>
                <input type="text" id="tujuan" name="tujuan" value="{{ old('tujuan', $sppd->tujuan) }}" required
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="date" id="tanggal_berangkat" name="tanggal_berangkat" value="{{ old('tanggal_berangkat', $sppd->tanggal_berangkat->format('Y-m-d')) }}" required
                ...
                <div>
                    ...
                    <input type="date" id="tanggal_kembali" name="tanggal_kembali" value="{{ old('tanggal_kembali', $sppd->tanggal_kembali->format('Y-m-d')) }}" required
            ...
            <div>
                ...
                <textarea id="keperluan" name="keperluan" rows="4" required
                          ...
                          placeholder="Jelaskan keperluan perjalanan dinas Anda...">{{ old('keperluan', $sppd->keperluan) }}</textarea>
                @error('keperluan')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
